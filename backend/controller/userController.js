...
export const addNewAdmin = catchAsyncError(async (req, res, next) => {
    ...
    const isRegistered = await User.findOne({ email });
    if (isRegistered) {
        return next(new ErrorHandler(`${isRegistered.role} with this email alerady registered`, 400));
    }
...
export const addNewDoctor = catchAsyncError(async (req, res, next) => {
    ...
    const { firstName, lastName, email, phone, Aadhar, dob, gender, password, doctorDepartment,
    } = req.body;
    if (
        !firstName || !lastName || !email || !phone || !Aadhar || !dob || !gender || !password ||
        !doctorDepartment || !docAvatar
    ) {
        return next(new ErrorHandler("Please Fill Full Form!", 400));
    }
    const isRegistered = await User.findOne({ email });
    ...
    const doctor = await User.create({
        firstName, lastName, email, phone, Aadhar, dob, gender, password, role: "Doctor",
        doctorDepartment,
        docAvatar: {
